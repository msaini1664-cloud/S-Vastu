          ...
          <div className="flex justify-between items-center h-20 sm:h-24">
            ...
            <nav className="hidden md:flex space-x-8 lg:space-x-10 items-center">
              {['Home', 'About', 'Services', 'Gallery', 'Blog', 'Contact'].map((item) => (
                <Link key={item} to={item === 'Home' ? '/' : `/${item.toLowerCase()}`} className="text-gray-700 hover:text-[#B8860B] font-semibold text-base transition-colors duration-300 relative group">
                  {item}
